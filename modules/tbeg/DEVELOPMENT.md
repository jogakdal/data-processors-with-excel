...
**Automatic number formatting is not applied to formula cells (cells with an `<f>` child element).** Since Excel determines formula result types at runtime, pre-applying formatting in TBEG can conflict with user intent (e.g., when the result is a string such as `IFERROR(VLOOKUP(...), "")`).

If number formatting is needed for a formula cell, specify the formatting directly on the template cell.

> **Implementation**: The `hasFormula` branch in `SheetXmlHandler.processCellElement` keeps the original style index instead of applying `styleMapping`.
