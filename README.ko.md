...
![템플릿](modules/tbeg/manual/images/rich_sample_template.png)
...
![결과](modules/tbeg/manual/images/rich_sample_result_ko.png)

변수 치환, 이미지 삽입, 반복 데이터 확장, 자동 셀 병합, 요소 묶음, 선택적 필드 노출, 수식 범위 조정, 조건부 서식 반영, 차트 데이터 반영까지 TBEG이 자동으로 처리합니다.

> 전체 코드와 템플릿 다운로드는 [종합 예제](modules/tbeg/manual/ko/examples/advanced-examples-kotlin.md#11-종합-예제) ([Java](modules/tbeg/manual/ko/examples/advanced-examples-java.md#11-종합-예제))를 참조하세요.
...
> 데이터 제공 방식(Map vs DataProvider), 출력 방식(generate/toStream/toFile) 비교 등 상세 분석은 [성능 벤치마크 상세](modules/tbeg/manual/ko/appendix/benchmark-results.md)를 참조하세요.
...
- [타 라이브러리 비교](modules/tbeg/manual/ko/appendix/library-comparison.md)
